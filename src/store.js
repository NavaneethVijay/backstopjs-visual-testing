...
async function processData() {
  try {
    ...
    const reports = await import(`../reports/${website.name}/jsonReport.json`);
    window.tests = reports.default;
  } catch (error) {
...
function parseLocalStorage(localStorageItem) {
  ...
  try {
    ...
    console.error(error);
    console.warn(
      "BackstopJS LocalStorage settings appear to be corrupted. Let me fix that for you."
    );
...
const persistedState = localState ? parseLocalStorage(localState) : false;
